...
class ServoXPlugin : public gz::sim::System,
...
public:
                 ...
                 gz::sim::EventManager &/*eventMgr*/) override
  {
    ...
    if (sdf && sdf->HasElement("arm_name")) {
      ...
      RCLCPP_INFO_STREAM(rclcpp::get_logger("servo_x_temp"), "✅ Read arm_name from SDF: '" << arm_name_ << "'");
    } else {
      arm_name_ = "arm_" + std::to_string(my_instance_id_);
      RCLCPP_WARN_STREAM(rclcpp::get_logger("servo_x_temp"), "⚠️ No arm_name in SDF, defaulting to: '" << arm_name_ << "'");
    }
    
    std::string node_name = "servo_x_" + arm_name_;
    node_ = rclcpp::Node::make_shared(node_name);
    
    // FIXED: Use the correct joint naming convention for X-servo
    joint_name_ = arm_name_ + "_x_servo_to_gimbal";  // ✅ FIXED: was "_x_servo_to_arm"
    position_topic_ = "/robot/" + arm_name_ + "/x_servo/position";
...
private:
  void findJoint(gz::sim::EntityComponentManager &ecm)
  {
    ...
    if (search_attempts_ > MAX_SEARCH_ATTEMPTS) {
      RCLCPP_ERROR(node_->get_logger(), 
                   "❌ %s: X-Servo joint '%s' search FAILED after %d attempts", 
    ...
    if (search_attempts_ % 20 == 1) {
      RCLCPP_WARN(node_->get_logger(), 
                  "⚠️ %s: Searching for X-servo joint '%s' (attempt %d/%d)", 
  ...
  {
    RCLCPP_INFO(node_->get_logger(), 
               "🔧 %s: Initializing joint components for X-servo...", 
               arm_name_.c_str());
  ...
  {
    RCLCPP_ERROR(node_->get_logger(), "=== 🔍 ALL JOINTS FOR %s X-SERVO DEBUG ===", arm_name_.c_str());
    auto joints = ecm.EntitiesByComponents(gz::sim::components::Joint());
  ...
  void controlServo(const gz::sim::UpdateInfo &info, gz::sim::EntityComponentManager &ecm)
  {
    ...
    if (++debug_counter % 100 == 0 && std::abs(error) > 0.01) {
      RCLCPP_DEBUG(node_->get_logger(), 
                  "🎛️ %s X-Servo: pos=%.3f, target=%.3f, error=%.3f, force=%.1f", 
                  arm_name_.c_str(), current_position_, target_position_, error, force);
    }
  }
};

int ServoXPlugin::instance_counter_ = 0;

// Register the plugin
GZ_ADD_PLUGIN(ServoXPlugin,
              gz::sim::System,
              ServoXPlugin::ISystemConfigure,
              ServoXPlugin::ISystemPreUpdate)

// Plugin aliases
GZ_ADD_PLUGIN_ALIAS(ServoXPlugin, "servo_x_plugin")
GZ_ADD_PLUGIN_ALIAS(ServoXPlugin, "front_right_servo_x")
GZ_ADD_PLUGIN_ALIAS(ServoXPlugin, "front_left_servo_x")
GZ_ADD_PLUGIN_ALIAS(ServoXPlugin, "back_right_servo_x") 
GZ_ADD_PLUGIN_ALIAS(ServoXPlugin, "back_left_servo_x")